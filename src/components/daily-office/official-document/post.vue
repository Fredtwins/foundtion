<template>
  <div class="post">
    ...
    <div class="top">
      <Button v-if="sendShow" type="success" @click="send">
        <Icon type="paper-airplane"></Icon>发送
      </Button>
      <Button v-if="circulatedShow" type="success" @click="circulated">
        <Icon type="paper-airplane"></Icon>传阅
      </Button>
      <Button v-if="draftShow" @click="draft">存草稿</Button>
    ...
    <div class="center">
      ...
      <div class="fujian divStyle">
        ...
        <input v-else="filename" v-model="model.attachment_name">
      </div>
      <div class="textarea">
        <quill-editor v-model="model.deo_detail" :options="{placeholder: '请输入'}"></quill-editor>
      </div>
    </div>
    <div class="bottom">
      <Button
...
import { uploadApi } from 'api/daily-office'
...
export default {
  ...
  created () {
    if (this.status === '草稿') {
      this.circulatedShow = false
      this.sendShow = true
      this.draftShow = true
    } else if (this.status === '收文') {
  ...
  methods: {
    ...
    ok () {
      ...
      this.model.release_department_name = getLocalStorage('info').department_name
      this.model.release_department_level = getLocalStorage('info').department_level
      this.model.release_department_level = getLocalStorage('info').department_level
      this.model.release_department_code = getLocalStorage('info').department_code
      if (this.circulatedShow == true) {
        this.model.receive_department = this.model.receive_department.concat(this.resultArr)
      } else {
        this.model.receive_department = this.resultArr
      }
      if (this.file) {
        let form = new FormData()
        form.append('file', this.file)
