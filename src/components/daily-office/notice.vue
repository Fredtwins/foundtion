  ...
  <div class="notice">
    ...
    <div class="addPage" v-show="addPageShow">
      ...
      <div class="insertInfo">
        <Form ref="formItem" :model="formItem" :rules="ruleValidate">
          <table class="ed-table disabled-color">
            <tbody>
              <tr>
                <td class="ed-label"><span class="font-color">* </span>标题</td>
                ...
                <td class="ed-content">
                  <FormItem style="margin: 0">
                    <!-- <Select v-model="formItem.status" placeholder="请选择" style="width:200px" disabled>
                        <Option value="草稿">草稿</Option>
                        <Option value="发布">发布</Option>
                    </Select> -->
                    <Input disabled v-model="formItem.status" placeholder="请输入" style="width:80%"></Input>
                  </FormItem>
                </td>
              </tr>
              <tr style="height:80px">
                <td class="ed-label"><span class="font-color">* </span>通知公告内容</td>
                <td class="ed-content" colspan="5">
                  <FormItem style="margin:0" prop="notice_desc">
                    <Input type="textarea" :disabled="formDisabled" v-model="formItem.notice_desc" placeholder="请输入" style="width:95%"></Input>
                  </FormItem>
                </td>
              </tr>
              <tr>
                ...
                <td class="ed-content upload" colspan="5">
                  <div>
                    <Upload v-if="!formDisabled" :before-upload="handleUpload" action="http://192.168.1.138:8080/sanfangstruts/oa/sf_notice/upload?systemId=1000010">
              ...
              <tr>
                ...
                <td class="ed-content" colspan="5">
                  <div class="demo-upload-list" v-for="item in uploadList">
                    <template v-if="uploadList.length">
                      ...
                      <div class="demo-upload-list-cover">
                        ...
                        <Icon v-if="!formDisabled" type="ios-trash-outline" @click.native="handleRemove(item)"></Icon>
                      </div>
                    </template>
                    <template v-else>
                  ...
                  <Upload v-if="!formDisabled" ref="upload" :show-upload-list="false" :default-file-list="defaultList" :on-success="handleSuccess" :format="['jpg','jpeg','png']"
      ...
      <div class="save pull-right">
        <Button v-if="showBtn" type="primary" size="large" @click="save('formItem')" style="margin-right:15px">保存</Button>
...
export default {
  ...
  mounted () {
    ...
    if (this.NOTICEID) {
      ...
      this.uploadList = this.NOTICEID.picture
    }
  },
  methods: {
    ...
    // 点击发布
    release () {
      if (this.releaseId && this.releaseId.length) {
        let obj = {
          _ids: this.releaseId,
          status: '发布'
        }
    ...
    ...mapMutations(['SET_STATUS', 'SET_NOTICEID']),
    // 点击修改
    edit (dataObj) {
      this.showBtn = true
      this.formDisabled = false
    ...
    // 点击详情
    details (dataObj) {
      // this.$router.push(`/notice-details/${dataObj._id}`)
      window.open(`${window.location.pathname}#/notice-details/${dataObj._id}`)
      // this.formDisabled = true
      // this.SET_STATUS(true)
      // this.showBtn = false
      // this.filename = "";
