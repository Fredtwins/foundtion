<template>
  <div class="tiweiweihu">
    ...
    <div class="edit">
      <div class="title">
        <span class="titlelefy"></span>
        <span>位置：</span>
        <Breadcrumb separator=">">
          <BreadcrumbItem>部门管理</BreadcrumbItem>
          <BreadcrumbItem>部门数据维护</BreadcrumbItem>
          <BreadcrumbItem to="/home/depManagement/jiuyuan">救援数据维护</BreadcrumbItem>
          <BreadcrumbItem>
            <a href="javascript:;" @click="cancel">救援数据</a>
          </BreadcrumbItem>
        ...
        <Form ref="formItem" :model="formItem" :label-width="0">
          <table class="ed-table">
            <tbody>
              ...
              <tr>
                <td class="ed-label">来源</td>
                <td class="ed-content">
                  <FormItem label prop="people" style="margin: 0">
                    <Input placeholder="请输入" style="width:80%" v-model="formItem.people"></Input>
                  </FormItem>
                </td>

                <td class="ed-label">联系电话</td>
                <td class="ed-content">
                  <FormItem label prop="phone" style="margin: 0">
                    <Input placeholder="请输入" style="width:80%" v-model="formItem.phone"></Input>
                  </FormItem>
                </td>
              </tr>
            </tbody>
...
<script>
import { getjiuyuanList } from 'api/shujuage'
...
export default {
  ...
  methods: {
    ...
    cancel () {
      this.$router.push('/home/depManagement/jiuyuan')
    },
    // 表格显示
    _getuserlist (search) {
      search = {
      ...
      if (getLocalStorage('id') !== 'false') {
        getjiuyuanList(search).then(res => {
          if (res.code === ERR_OK) {
