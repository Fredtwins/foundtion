  ...
  <div class="temporary">
    <div class="titles">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/temporary">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据上报</BreadcrumbItem>
        <BreadcrumbItem>临时数据模版</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索  -->
    <Form ref="formInline" inline :label-width="60">
      <Form-item label="表单名称">
        <Input v-model="formInline.formname" :maxlength="16" placeholder="请输入表单名称"></Input>
      </Form-item>
      <div class="searchBtn">
...
<script>
import { getUserList, getDelList } from 'api/temporary'
...
export default {
          ...
          render: (h, params) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    let data = cloneObj(params.row, params.inex)
                    this.edit(data)
                  }
                }
              }, '编辑'),
  ...
  methods: {
    ...
    submitsearch (page) {
      ...
      if (search.formname) {
        search.formname = likeStrSearch(search.formname)
      }
      this._getUserList(this.current, search)
    },
    ...
    addlist () {
      // 将id设置为false
      getLocalStorage('temporaryid', 'false')
      this.$router.push('/home/depManagement/deplist/temporarypage')
    },
    // 点击编辑
    edit (dataObj) {
      // 将id设置为
      getLocalStorage('temporaryid', dataObj._id)
      this.$router.push('/home/depManagement/deplist/temporarypage')
    },
    // 点击删除
    del (dataObj) {
      this.rowIng = cloneObj(dataObj)
      this.modal2 = true
    },
    // 删除成功
    _getDelList () {
