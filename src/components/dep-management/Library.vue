<template>
  <div class="Library">
    ...
    <template slot="content">
      <!--新增，修改  -->
      <Modal v-model="modal1" class="modalz modal">
        ...
        <Form :model="formItem" ref="formItem" :rules="ruleValidate" :label-width="120">
          ...
          <Form-item label="部门名称(简称)">
            <Input v-model="formItem.department_name" disabled placeholder="请输入部门名称"></Input>
          </Form-item>
          <Form-item label="部门编号">
            <Input v-model="formItem.department_code" disabled placeholder="请输入部门编号"></Input>
          </Form-item>
          <Form-item label="部门级别">
            <Input v-model="formItem.department_level" disabled placeholder="请输入部门级别"></Input>
          </Form-item>
          ...
          <Form-item label="详细描述">
            <Input
              v-model="formItem.instruction_desc"
              type="textarea"
              :autosize="{minRows: 10,maxRows: 20}"
              placeholder="请输入指令详细描述"
            ></Input>
          </Form-item>
        </Form>
        <div slot="footer">
          <Button v-if="btnSave" type="primary" @click="_LibraryaddList('formItem')">保存</Button>
...
import { LilistRules } from 'common/js/rules'
...
export default {
  ...
  methods: {
    ...
    selected (value) {
      ...
      obj[str] = this.formItem.department_code
      // 执行部门下拉框
    ...
    // 保存按钮
    _LibraryaddList (name) {
      console.log(name)
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.department.map(item => {
            if (item.name === this.formItem.name) {
              return this.formItem.code = item.code // eslint-disable-line
            }
          })
...
<style scoped lang="scss">
	...
	.modal .ivu-modal .ivu-modal-content .ivu-modal-body {
		margin-top: 20px;
		height: 616px;
	}
</style>
