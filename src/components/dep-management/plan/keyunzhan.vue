  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/keyunzhan">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>客运站数据</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
...
<script>
import { keyunzhanThead } from 'common/js/tables'
...
export default {
  data () {
    return {
      loading: false,
      tableThead: keyunzhanThead(this),
      tableTbody: [],
      total: 1,
      current: 1,
      modal2: false,
  ...
  methods: {
    ...
    _getuserList (page, search) {
      this.loading = true
      getkeyunzhanList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
    ...
    edit (dataObj) {
      ...
      this.$router.push('/home/depManagement/keyunzhan/keyunzhanweihu')
    },
    // 删除
    del (row) {
