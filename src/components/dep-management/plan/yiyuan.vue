  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/yiyuan">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>医院数据</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
    <Form ref="formInline" inline :label-width="60">
      ...
      <Form-item label="类型">
        <Input v-model="formInline.type" :maxlength="16" placeholder="请输入"></Input>
      </Form-item>
      <Form-item label="地点">
...
<script>
import { yiyuanThead } from 'common/js/tables'
import { getyiyuanList } from 'api/shujuage'
...
export default {
  ...
  methods: {
    ...
    add () {
      ...
      this.$router.push('home/depManagement/shangchang/shangchangweihu')
    },
    // 搜索
    submitsearch (page) {
      let search = {}
      ...
      if (this.formInline.type) {
        search.type = likeStrSearch(this.formInline.type)
      }
    ...
    _getuserList (page, search) {
      this.loading = true
      getyiyuanList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
    ...
    edit (dataObj) {
      ...
      this.$router.push('/home/depManagement/yiyuan/yiyuanweihu')
    },
    // 删除
    del (row) {
