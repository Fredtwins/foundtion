  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/paishuisheshi">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>排水设施数据</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
...
<script>
import { paishuisheshiThead } from 'common/js/tables'
...
export default {
  data () {
    return {
      loading: false,
      tableThead: paishuisheshiThead(this),
      tableTbody: [],
      total: 1,
      current: 1,
      modal2: false,
  ...
  methods: {
    ...
    _getuserList (page, search) {
      this.loading = true
      getsheshiList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
    ...
    edit (dataObj) {
      ...
      this.$router.push('/home/depManagement/paishuisheshi/paishuisheshiweihu')
    },
    // 删除
    del (row) {
