  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/yinhan">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>银行数据</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
...
<script>
import { yinhanThead } from 'common/js/tables'
import { getyinhanList } from 'api/shujuage'
...
export default {
  ...
  methods: {
    ...
    add () {
      ...
      this.$router.push('/home/depManagement/yinhan/yinhanweihu')
    },
    // 搜索
    submitsearch (page) {
      let search = {}
    ...
    _getuserList (page, search) {
      this.loading = true
      getyinhanList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
