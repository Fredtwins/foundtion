  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/shangchang">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>商场数据</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
...
<script>
import { shangchangThead } from 'common/js/tables'
...
export default {
  data () {
    return {
      loading: false,
      tableThead: shangchangThead(this),
      tableTbody: [],
      total: 1,
      current: 1,
      modal2: false,
  ...
  methods: {
    ...
    _getuserList (page, search) {
      this.loading = true
      getshangchangList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
