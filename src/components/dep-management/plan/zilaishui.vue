  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/zilaishui">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>自来水厂数据</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
    <Form ref="formInline" inline :label-width="60">
      ...
      <Form-item label="镇街">
        <Input v-model="formInline.district" :maxlength="16" placeholder="请输入"></Input>
      </Form-item>
...
<script>
import { waterThead } from 'common/js/tables'
...
export default {
  ...
  methods: {
    ...
    submitsearch (page) {
      ...
      if (this.formInline.district) {
        search.district = likeStrSearch(this.formInline.district)
      }
    ...
    _getuserList (page, search) {
      this.loading = true
      getshuichangList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
    ...
    edit (dataObj) {
      ...
      this.$router.push('/home/depManagement/zilaishui/zilaishuiweihu')
    },
    // 删除
    del (row) {
