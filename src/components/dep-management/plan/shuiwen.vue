  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/shuiwen">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>水文数据维护</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
    <Form ref="formInline" inline :label-width="60">
      <Form-item label="名称">
        <Input v-model="formInline.admauth" :maxlength="16" placeholder="请输入"></Input>
      </Form-item>
      <Form-item label="地点">
...
<script>
import { shuiwenThead } from 'common/js/tables'
...
export default {
  ...
  methods: {
    ...
    submitsearch (page) {
      let search = {}
      if (this.formInline.admauth) {
        search.admauth = likeStrSearch(this.formInline.admauth)
      }
    ...
    _getuserList (page, search) {
      this.loading = true
      getshuiwenList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
    ...
    edit (dataObj) {
      ...
      this.$router.push('/home/depManagement/shuiwen/shuiwenweihu')
    },
    // 删除
    del (row) {
