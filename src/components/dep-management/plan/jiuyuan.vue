  ...
  <div class="school">
    <div class="title">
      ...
      <Breadcrumb separator=">">
        <BreadcrumbItem to="/home/depManagement/jiuyuan">部门管理</BreadcrumbItem>
        <BreadcrumbItem>部门数据维护</BreadcrumbItem>
        <BreadcrumbItem>救援数据</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!--搜索-->
...
<script>
import { jiuyuanThead } from 'common/js/tables'
...
export default {
  ...
  methods: {
    ...
    _getuserList (page, search) {
      this.loading = true
      getjiuyuanList(page, search).then(res => {
        if (res.code === ERR_OK) {
          this.loading = false
    ...
    edit (dataObj) {
      ...
      this.$router.push('/home/depManagement/jiuyuan/jiuyuanweihu')
    },
    // 删除
    del (row) {
