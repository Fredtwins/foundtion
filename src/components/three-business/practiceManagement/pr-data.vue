<template>
  <div class="pr-data">
    ...
    <div class="search">
      <span class="search_title">台风编号</span>
      <Input
        placeholder="请输入"
        v-model="searchVal.tfbh"
...
export default {
  ...
  methods: {
    getList () {
      this.loading = true
      prDataApi(this.postObj).then(res => {
        this.loading = false
        if (res.code === '0000') {
    ...
    search () {
      ...
      if (this.searchVal.tfbh) {
        this.postObj.searchValue.tfbh = likeStrSearch(this.searchVal.tfbh)
      }
      this.getList()
    },
    // 分页
    ...
    // 点击保存
    save () {
      prDataApi(this.apiType, this.formItem).then(res => {
        if (res.code === '0000') {
          successNotice(res.message)
          this.getList()
        } else {
