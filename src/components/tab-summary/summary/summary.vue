  ...
  <div class="defense-statical">
    ...
    <div class="title">
      ...
      <Breadcrumb separator=">">
        ...
        <BreadcrumbItem>应急响应总结统计</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <!-- <div class="search">
    ...
    <div v-if="addPageShow" class="addPage">
      ...
      <add-page
        @cancel="cancel"
        :defenseModel="defenseObj"
        :affectedModel="affectedObj"
        :transferdModel="transferdObj"
        :defenseSize="defenseSize"
        :affectedSize="affectedSize"
        :transferdSize="transferdSize"
        :data="formItem"
      ></add-page>
    </div>
  </div>
...
import { summaryThead } from 'common/js/table'
import { defensedetailsApi, detailsApi, affecteddetailsApi } from 'api/tab-summary'
import { getErmList } from 'api/er-management'
...
export default {
  ...
  methods: {
    ...
    details (obj) {
      ...
      this.spinShow = true
      this.formItem = obj
      defensedetailsApi({response_no: obj.response_no}).then(res => {
        this.addPageShow = true
        this.spinShow = false
        if (res.code === '0000') {
          this.defenseObj = res.result.result
          if (res.result.totalSize === 0) {
            this.defenseObj.push(
              {
                department_name: '-',
                type: '-',
                count: '-'
            ...
            this.defenseSize = res.result.totalSize + 1
          }
        }
      })
      affecteddetailsApi({response_no: obj.response_no}).then(res => {
        this.addPageShow = true
        this.spinShow = false
        if (res.code === '0000') {
          this.affectedObj = res.result.result
          if (res.result.totalSize === 0) {
            this.affectedObj.push(
              {
                department_name: '-',
                disaster_type: '-',
                death: '-',
            ...
            this.affectedSize = res.result.totalSize + 1
          }
        }
      })
      detailsApi({response_no: obj.response_no}).then(res => {
        this.addPageShow = true
        this.spinShow = false
        if (res.code === '0000') {
          this.transferdObj = res.result.result
          if (res.result.totalSize === 0) {
            this.transferdObj.push(
              {
                department_name: '-',
                stattype: '-',
                count: '-',
            ...
            this.transferdSize = 2
          } else {
            this.transferdSize = res.result.totalSize + 1
          }
        }
      })
    }
