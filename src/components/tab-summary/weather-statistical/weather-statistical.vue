  ...
  <div class="defense-statical">
    ...
    <div class="title">
      ...
      <Breadcrumb separator=">">
        ...
        <BreadcrumbItem>气象数据查询与统计</BreadcrumbItem>
      </Breadcrumb>
    </div>
    <Tabs type="card" :animated="false" @on-click="jump">
      <TabPane label="温度" name="1">
        <div class="search">
...
<script>
import { weatherThead, rainThead } from 'common/js/table'
import { weather, rainApi } from 'api/tab-summary'
import { mapGetters, mapMutations } from 'vuex'
import { errorNotice } from 'common/js/dom'
import { weatherModule } from 'common/js/module'
...
export default {
  ...
  data () {
    return {
      addModuleData: weatherModule,
      spinShow: false,
      tableThead: weatherThead(this),
      tableTbody: [],
      formItem: {},
      rainTableThead: rainThead(this),
      rainTableTbody: [],
      searchVal: {},
      total: 0,
      loading: false,
  ...
  methods: {
    ...
    getRainList () {
      this.loading = true
      rainApi(this.pObj).then(res => {
        this.loading = false
        if (res) {
          if (res.code === '0000') {
            this.rainTableTbody = res.result.result
            this.total = res.result.totalSize
          }
        } else {
          errorNotice('服务器未连接，')
    ...
    search () {
      ...
      if (this.tabName === '1') {
        ...
        this.pObj.searchValue = cloneObj(this.searchVal)
        if (this.searchVal.stn_name) {
          this.pObj.searchValue.stn_name = likeStrSearch(this.searchVal.stn_name)
        }
        this.getRainList()
      }
    },
