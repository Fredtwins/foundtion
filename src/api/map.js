...
export function getLayer (item, option) {
  let fn
  let layerName = typeof item === 'string' ? item : item.layer
...
// 创建ImageStatic Source
export function createdImageStaticSource (obj = {}) {
...
// 雨 Layer
function rainLayer (item) {
  let url = `${mapHttpUrl20463}ReqMode24Service?mode=rain1h&area=nanhai`

  return ajaxGet1(url).then(res => {
    let result = [],
      code = '',
      layer = ''
    ...
    if (res.contents && res.contents.length) {
      ...
      if (result.length) {
        ...
        let source = createdImageStaticSource({
          url: result[0].serviceResult,
...
export function createdWindFeatures (option = {}) {
  let features = []

  option.result.map(items => {
    let feature = createFeatures('lng', 'lat', [items], `${Static}img/windIcon/${items.level}/${items.level}_${items.wd10}.png`, option.windScaleList[option.zoom])
    features.push(...feature)
  })
...
// 风力风向 Layer
function windLayer (item, option) {
  let url = `${mapHttpUrl20463}ReqWindbarGraphService?mode=wind10m`
  return ajaxGet1(url).then(res => {
    let result = [],
      features = [],
      code = '',
    ...
    if (res.contents && res.contents.length) {
      ...
      if (result.length) {
        features = createdWindFeatures({
          result: result[0].data,
          windScaleList: res.scale_list,
          zoom: option.zoom
        })
      }

      layer = createFeaturesLayer(item.layer, features)
    }
...
// 云图 Layer
function cloudLayer () {
  let url = `${dataMapHttpUrl}fullservice/http/metorImage?imageType=cloudImage`
...
// 雷达图 Layer
function radarLayer () {
  let url = `${dataMapHttpUrl}fullservice/http/metorImage?imageType=radarImage`
...
function refugeLayer (item) {
  let url = `${dataMapHttpUrl}fullservice/http/refugeInfo`
...
function emergencyLayer (item) {
  let url = `${dataMapHttpUrl}fullservice/http/suppliesWarehouseInfo`
...
function emergencySquadLayer (item) {
  let url = `${dataMapHttpUrl}fullservice/http/emergencySquad`
...
function rescueTeamLayer (item) {
  let url = `${dataMapHttpUrl}fullservice/http/rescueTeam`
...
function massHeatLayer () {
  let url = `${dataMapHttpUrl}fullservice/http/peopleHeadHot`
...
function liabilityPersonLayer (item) {
  let url = `${dataMapHttpUrl}fullservice/http/liabilityPerson`
...
function defenseExpertLayer (item) {
  let url = `${dataMapHttpUrl}fullservice/http/defenseExpert`
